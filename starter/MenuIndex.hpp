...
class NameDirectory {
    ...
    public:
        NameDirectory(MenuModel const& model, MenuNames const& names)
            : _courses(_courseMap(names)) {
        ...
        int dishIndexOf(int courseIdx, std::string const& name) const {
            if (courseIdx < 0 || courseIdx >= static_cast<int>(_dishes.size())) return -1;
...
inline std::vector<int> settlingOrder(MenuModel const& model) {
    ...
    IndexedMinHeap heap;
    for (int c = 0; c < C; ++c)
        heap.insert(static_cast<double>(neighbors[c].size()), c);
