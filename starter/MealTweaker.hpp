...
class MealTweaker {
    ...
    public:
        MealTweaker(MenuModel const& model, MenuNames const& names, Meal startingMeal)
            : _model(model),
              _dir(model, names),
              _meal(static_cast<Meal&&>(startingMeal)),
              _tableScore(static_cast<std::size_t>(model.numTables())),
              _score(0.0) {
        ...
        bool alterDish(std::string const& courseName, std::string const& dishName) {
            ...
            if (_model.isChosen(c)) return false;          // locked course
            if (_meal.dishFor(c) == d) return false;       // already selected
